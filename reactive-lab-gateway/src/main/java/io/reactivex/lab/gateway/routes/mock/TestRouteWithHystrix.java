...
public class TestRouteWithHystrix {
    ...
    public static Observable<Void> handle(ClientRegistry clientRegistry, HttpServerRequest<ByteBuf> request,
        ...
        Observable<List<BackendResponse>> acd = new MockServiceCommand(id, 2, 50, 50, clientRegistry).observe()
                ...
                .<List<BackendResponse>> flatMap(responseA -> {
                    Observable<BackendResponse> responseC = new MockServiceCommand(responseA.getResponseKey(), 1, 5000, 80, clientRegistry).observe();
                    Observable<BackendResponse> responseD = new MockServiceCommand(responseA.getResponseKey(), 1, 1000, 1, clientRegistry).observe();
                    return Observable.zip(Observable.just(responseA), responseC, responseD, (a, c, d) -> Arrays.asList(a, c, d));
                });

        Observable<List<BackendResponse>> be = new MockServiceCommand(id, 25, 30, 15, clientRegistry).observe()
                // Eclipse 20140224-0627 can't infer without this type hint even though the Java 8 compiler can
                .<List<BackendResponse>> flatMap(responseB -> {
                    Observable<BackendResponse> responseE = new MockServiceCommand(responseB.getResponseKey(), 100, 30, 4, clientRegistry).observe();
